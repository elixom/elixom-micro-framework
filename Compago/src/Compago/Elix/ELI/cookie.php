...
class ELI_cookie implements ArrayAccess, Iterator, Countable
{
    ...
    public static function setLifeTime($time=0, $relative=false){
        if($time ==0){
            $time = time() + 3600;
        }elseif($time < 0){
            $time = time() + $time;
    ...
    public static function setSecure($secure=false){
        $this->_options['secure'] = $secure;
    }
    public static function setDomain($dom='', $allowSubDomains=true){
        if(empty($dom)){
