...
class ELI_password
{
    public static function gem($length=8,$charset=null){
        $length = (int)$length;
    ...
    public static function generate($letters = 8,$numbers=2,$symbols=0,$charset=null)
    {	
        if(is_array($letters)){
            $a = $letters;
    ...
    public static function get_salt($hashed_password) {
        $x = explode('$',$hashed_password,3);
        return $x[1];
    }
    
    public static function salt($length=9, $entropy=3) {
    ...
    public static function hash1($password,$salt=null) {
        if(empty($salt)){
            $salt = self::salt(10);
    ...
    public static function hash2($password,$salt=null) {
        if(empty($salt)){
            $salt = self::salt(10);
