...
class randomgenerator{
    public static function getGenerator($options = array()){
    ...
    public static function sentence($words =0, $max_letter_length=0) {
    ...
    public static function paragraph($sentences=5,$max_letter_length=0) {
...
*/
    public static function loripsum($params=array(3,'plaintext','prude')) {
        if(is_array($params)){
            $p = implode('/',$params);
        }else{
            $p = (string)$params;
    ...
    public static function name($fullname=false,$gender=0) {
        $r = self::random(2);
        if($r == 1){
            $l = self::random(8);
            $r = 2+self::random(9);
    ...
    public static function firstname($gender=0) {
        if($gender==1){
            $n = self::_maleNames();
        }elseif($gender==2){
            $n = self::_femaleNames();
    ...
    public static function dateofbirth($minAge=10,$maxAge=0) {
    ...
    public static function date($daysAgo=15,$daysFuture=15) {
        $ts = time();
    ...
    public static function time($minSeconds=0,$maxSeconds =86400, $return_seconds =false) {
        if(func_num_args()==0){
    ...
    public static function getDateObject($daysAgo=15,$daysFuture=15) {
        $ts = time();
        $tsMin = $ts - (86400 * $daysAgo);
        $tsMax = $ts + (86400 * $daysFuture);
        $ts = mt_rand($tsMin,$tsMax);
