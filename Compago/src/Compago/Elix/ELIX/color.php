...
// http://www.w3.org/TR/WCAG20/#relativeluminancedef
    ...
    public function getContrastYIQ(){
        $ci = clone $this;
    	if($this->yiq() >= 128){
            $ci->setRgb(0,0,0);
        }else{
    ...
    public function getContrastRGB(){
        $ci = clone $this;
        $r = ($this->r < 128) ? 255 : 0;
        $g = ($this->g < 128) ? 255 : 0;
        $b = ($this->b < 128) ? 255 : 0;
...
public function is_hex($hexValue){ 
    if(strtolower($hexValue) == dechex(hexdec($hexValue))) 
        return true; 
    return false; 
}
public function hex2rgb($color,  $seperator = '')
{
...
public function rgb2hex($r,$g=0,$b=0){
...
public function array2hex($arr){
    ...
    public function rgbFx2Hex($color){
...
public function matchCommonColor($color){
    ...
    public function commonColors($color){
...
public function rgb_to_cmyk($r,$g,$b){
...
public function rgb2hsl($rgbtrio) {
...
public function hsl2rgb($hsltrio) {
    $h = $hsltrio[0];
    $s = $hsltrio[1];
    $L = $hsltrio[2]/100;
