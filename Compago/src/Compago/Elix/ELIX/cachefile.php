...
class CACHEFILE
{
    ...
    public static function version(){return static::$VERSION ;}
    protected $path = null;
    protected $ttl = 60;
    protected $time = 0;
    protected $data = null;
    ...
    public function valid($unlink=true) {
        if(empty($this->path)) return null;
        if($this->expired()){
    ...
    public function useable($unlink=true){
        trigger_error('incorrect spelling use usable');
        return $this->usable($unlink);
    }
    public function usable($unlink=true) {
        if(empty($this->path)) return null;
        if($this->expired()){
            if($unlink && file_exists($this->path)) @unlink($this->path);
            return false;
    ...
    public function until($d='midnight tomorrow') {
