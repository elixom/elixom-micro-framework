...
class Index extends \Compago\Database\Core\Attribute{
    public function __construct($options=array()) {
        if(is_array($options)){
            $this->setIndexOptions($options);
            if(!isset($options['attribute_type'])){
                $this->options['attribute_type'] = 'INDEX';
            }
            if(isset($options['key_part'])){
                $this->setKeyPart($this->options['key_part']);
            }
        }
    }
        ...
        case 'ALTER':
            ...
            if($this->mode=='DROP'){
                if($this->attribute_type =='INDEX' || $this->attribute_type=='KEY'){
                    ...
                    if($this->name){
                        return "DROP {$this->attribute_type} `{$this->name}`";
                    }
                }
                if($this->attribute_type =='COLUMN'){
                    if($this->name){
